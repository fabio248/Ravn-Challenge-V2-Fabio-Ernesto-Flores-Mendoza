...
productRouter.post('/', fileUpload({ createParentPath: true }), // Middleware for handling file upload
...
validatorHandler(createProductSchema, 'body'), passport.authenticate('jwt', { session: false }), //Check if autenticated
checkRoles('MANAGER'), //Check if manager user
createProduct);
productRouter.put('/:id', passport.authenticate('jwt', { session: false }), checkRoles('MANAGER'), validatorHandler(getProductSchema, 'params'), validatorHandler(updateProductSchema, 'body'), updateProduct);
productRouter.delete('/:id', passport.authenticate('jwt', { session: false }), checkRoles('MANAGER'), validatorHandler(getProductSchema, 'params'), deleteProduct);
